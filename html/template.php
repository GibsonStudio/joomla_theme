...
<config>
  <fields name="params">
   ...
   <field name="footertext" type="textarea" default="Custom text here" label="Custom text here" description="Custom text here" rows="1" cols="60"/> <field name="analyticsdisable" type="radio" default="1" label="Google analytics"><option value="0">Disable</option><option value="1">Enable</option></field> 	*/ if (JURI::base() == JURI::current()) {echo '<div class="binary">Copyright ' . date('Y'); if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . '</div>';}} else {echo '<div class="binary">Copyright ' . date('Y'); if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . '</div>';}} /* 
...
*/
?><?php /*  <field name="slide8" type="media" label="Upload slider image 8 (size must be 980px × 350px)" description="select an image" />  	<field type="spacer" name="block2" hr="true" />	 </fieldset></fields></config></extension>*/?> <a class='design' href='http://www.ukbinaryoptions.net/finpari/' target='_blank'>finpari reviews</a><a class='design' href='http://www.tradebinaryoptions.net/reviews/pwrtrade/' target='_blank'>pwrtrade review</a>